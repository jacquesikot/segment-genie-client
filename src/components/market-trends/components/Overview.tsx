...
const Overview = ({ data }: OverviewProps) => {
  ...
  const currentState = data.marketOverview?.currentState || 'No market overview data available.';
  const dominantTrends = data.marketOverview?.dominantTrends || [];
    ...
    <div className="space-y-6">
      ...
      <Card className="shadow-md dark:bg-gray-900 border-0 overflow-hidden sm:p-0">
        ...
        <div className="text-sm">
          <p className="text-gray-700 dark:text-gray-300 leading-relaxed mb-4">{currentState}</p>

          {dominantTrends.length > 0 && (
            <div className="mt-6">
              <h3 className="font-medium text-sm mb-3 text-gray-800 dark:text-gray-200">Dominant Trends</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                {dominantTrends.map((trend, index) => (
                  <div
                    key={index}
